...
namespace TestProjectAPI
{
    ...
    public class UnitTest1
    {
        ...
        {
            var result = ChamaApiPost("https://localhost:0000/api/List").Result; // Pegar url ao rodar no swagger
        ...
        public void GetToken()
        {
            ...
            string urlApiGeraToken = "https://localhost:0000/api/CriarTokenIdentity"; // Pegar url ao rodar no swagger

            using (var cliente = new HttpClient())
            {
                string login = ""; // Criado ao gerar usuário
                string senha = ""; // Criado ao gerar usuário
                var dados = new
                {
                    email = login,
                    senha = senha,
